...
exports.checkStaleLocations = exports.onStopCreated = exports.healthCheck = void 0;
 ...
 */
exports.onStopCreated = functions.firestore
    ...
    try {
        ...
        const userDocs = await Promise.all(targetMemberUids.map((uid) => admin.firestore().collection('users').doc(uid).get()));
        const pushPayload = {
            notification: {
    ...
    try {
        ...
        for (const tripDoc of tripsSnap.docs) {
            ...
            for (const locDoc of locationsSnap.docs) {
                ...
                const normalizedMillis = typeof rawUpdatedAt === 'number'
                    ? rawUpdatedAt
                ...
                if (!normalizedMillis || isNaN(normalizedMillis))
                    continue;
                const elapsedMs = now - normalizedMillis;
                if (elapsedMs < LOCATION_ALERT_AFTER_MS)
                    continue;
