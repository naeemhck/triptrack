...
Deno.serve(async (request) => {
  ...
  if (!eventId) return json({ error: 'Missing member event' }, 400);

  const admin = adminClient();
  const { data: event, error } = await admin
    .from('trip_member_events')
    .select('id,trip_id,user_id,event_type,profiles!trip_member_events_user_id_fkey(display_name)')
    .eq('id', eventId)
    .single();
  ...
  const { data: preferences, error: preferenceError } = memberIds.length
    ? await admin
        .from('trip_notification_preferences')
        .select('user_id,member_left_enabled')
        .eq('trip_id', event.trip_id)
        .in('user_id', memberIds)
    : { data: [], error: null };
  if (preferenceError) return json({ error: 'Preference query failed' }, 500);
  const disabled = new Set(
    (preferences || [])
      .filter((row) => row.member_left_enabled === false)
      .map((row) => row.user_id),
  );
  const recipientIds = memberIds.filter((userId) => !disabled.has(userId));
  ...
  const { data: tokens } = await admin
    ...
    .eq('enabled', true);
  const name = event.profiles?.display_name || 'A member';
  const removed = event.event_type === 'member_removed';
  let delivered = 0;
    ...
    try {
      const response = await fetch(EXPO_PUSH_URL, {
        ...
        body: JSON.stringify({
          ...
          title: removed ? 'Trip member removed' : 'Trip member left',
          body: `${name} ${removed ? 'was removed from' : 'left'} the trip.`,
          data: { tripId: event.trip_id, memberUserId: event.user_id, eventType: event.event_type },
        }),
      });
