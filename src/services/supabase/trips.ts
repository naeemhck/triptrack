...
export async function listTrips(): Promise<Trip[]> {
  const { data: memberships, error: membershipError } = await supabase.from('trip_members').select('trip_id,user_id');
  if (membershipError) throw membershipError;
  const tripIds = [...new Set((memberships || []).map((row) => row.trip_id))];
  if (tripIds.length === 0) return [];
  const { data, error } = await supabase.from('trips').select('*').in('id', tripIds).order('created_at', { ascending: false });
  if (error) throw error;
  return (data || []).map((row) => mapTrip(row, (memberships || []).filter((m) => m.trip_id === row.id).map((m) => m.user_id)));
}
...
export async function getTripPreview(inviteCode: string): Promise<TripPreview> {
  const { data: joined, error: joinError } = await supabase.rpc('preview_trip', { p_invite_code: inviteCode });
  if (joinError) throw joinError;
  const tripRow = Array.isArray(joined) ? joined[0] : joined;
  const members = await listMembers(tripRow.id);
  return { trip: mapTrip(tripRow, members.map((m) => m.uid)), members, memberCount: members.length };
}
...
export async function joinTrip(inviteCode: string): Promise<Trip> {
  ...
  return mapTrip(row, members.map((m) => m.uid));
}

export async function listMembers(tripId: string) {
  const { data, error } = await supabase.from('trip_members').select(memberSelect).eq('trip_id', tripId).order('joined_at');
  if (error) throw error;
  return (data || []).map(mapMember);
}

export async function setSharing(tripId: string, enabled: boolean, mode: 'always' | 'foreground' | 'off' = enabled ? 'always' : 'off'): Promise<void> {
...
export async function runTripRpc(name: 'start_trip'|'end_trip'|'leave_trip', tripId: string): Promise<void> {
