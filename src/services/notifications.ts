...
export const registerForPushNotificationsAsync = async (uid: string): Promise<string | null> => {
  if (!Device.isDevice) {
    devLog('[Notification Service] Mock mode or Simulator detected — skipping push token registration.');
    return null;
  }
  ...
  try {
    ...
    if (!projectId) throw new Error('EXPO_PUBLIC_EAS_PROJECT_ID is required for Expo push registration.');
...
export const disablePushTokensForCurrentUser = async (uid: string): Promise<void> => {
  const { error } = await supabase.from('push_tokens').update({ enabled: false }).eq('user_id', uid);
  if (error) throw error;
};
...
export const setupNotificationResponseListener = (
  ...
  const navigateFromResponse = (response: Notifications.NotificationResponse): boolean => {
    ...
    if (!uuidPattern.test(tripId) || (stopId !== undefined && !uuidPattern.test(stopId))) return false;
